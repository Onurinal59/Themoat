...
let content = fs.readFileSync('src/components/CompanyAuditLab.tsx', 'utf-8');
...
content = content.replace(/\$\$\{([a-zA-Z0-9_]+)\.toFixed\(([0-9])\)\}M/g, '${formatCurrency($1 * 1000000)}');
content = content.replace(/\$\$\{([a-zA-Z0-9_]+)\}M/g, '${formatCurrency($1 * 1000000)}');

fs.writeFileSync('src/components/CompanyAuditLab.tsx', content, 'utf-8');
console.log("Fixed CompanyAuditLab!");
